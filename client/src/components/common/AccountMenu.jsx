...
const NotificationsIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M18 8a6 6 0 10-12 0v4l-2 2v1h16v-1l-2-2V8" />
...
export default function AccountMenu() {
  ...
  useEffect(() => {
    api.get('/account/kyc')
      .then(({ data }) => setKycStatus(data?.status || 'not_submitted'))
      .catch(() => { });
  }, []);
  ...
  const menuItems = MENU_ITEMS.map((item) => {
    if (item.to === '/account/kyc') {
      return {
        ...item,
        label: kycStatus === 'approved' ? 'KYC Verified' : 'KYC Verification',
        ...
        <>
          ...
          <div className="acct-panel">
            ...
            <nav className="acct-panel-nav">
              {menuItems.map(({ label, to, Icon, desc }) => (
                <button
                  key={to}
